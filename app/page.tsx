  ...
  return (
    <div className="flex min-h-screen flex-col bg-white">
      ...
      <main className="flex-grow">
        ...
        <section className="relative pt-6 pb-16 md:pt-16 md:pb-32 overflow-hidden">
          <div className="container mx-auto px-4 md:px-6 relative z-10 text-center">
            <div className="flex flex-col items-center space-y-4 md:space-y-6">
              ...
              <div className="mt-8 md:mt-12 w-full max-w-6xl mx-auto relative group text-left px-0 md:px-0">
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 md:gap-6">
                            ...
                            <div className="h-32 md:h-40 flex items-end gap-2 md:gap-3 px-1 md:px-2">
                              {[
                                ...
                                <div key={i} className="flex-1 flex flex-col items-center gap-2 h-full justify-end">
                                  <div style={{ height: bar.h }} className={`w-full bg-gradient-to-t ${bar.c} rounded-t-sm md:rounded-t-lg transition-all duration-500 relative`}>
                          ...
                          <div className="p-4 md:p-6 border rounded-xl md:rounded-2xl bg-[#0f172a] text-white shadow-xl relative overflow-hidden">
                             ...
                             <div className="p-3 bg-white/5 rounded-lg border border-white/10">
                               <p className="text-[10px] text-slate-400 mb-1 font-bold italic">Kapoor Family</p>
        ...
        <section id="detailed-features" className="py-16 md:py-24 bg-white border-t">
          <div className="container mx-auto px-4 md:px-6">
            <div className="text-center mb-12 md:mb-16">
              <h2 className="text-2xl md:text-4xl font-bold tracking-tight">Professional Toolkit for Every Agent</h2>
              <p className="mt-4 text-sm md:text-lg text-slate-500 max-w-2xl mx-auto">
        ...
        {/* === BRAND ORIGIN === */}
        <section className="py-16 md:py-24 bg-slate-50 border-t text-center">
          <div className="container mx-auto px-4">
            <p className="text-primary font-bold uppercase tracking-widest text-xs mb-4">Built by Pixalara</p>
            <h2 className="text-xl md:text-3xl font-bold mb-6 italic text-slate-800">"Digital Experiences. Engineered to Scale."</h2>
...
function SidebarItem({ icon: Icon, label, active = false, mini = false }: { icon: any, label: string, active?: boolean, mini?: boolean }) {
  return (
    <div className={`flex items-center gap-2 md:gap-3 px-3 py-2 md:py-2.5 rounded-lg transition-all shrink-0 ${active ? 'bg-primary/10 md:border-l-4 md:border-primary text-white' : 'text-slate-400 md:hover:bg-white/5'}`}>
...
function StatCard({ icon: Icon, label, value, color }: { icon: any, label: string, value: string, color: string }) {
  return (
    <div className="p-4 md:p-5 border rounded-xl md:rounded-2xl bg-white shadow-sm">
      ...
      <h4 className="text-lg md:text-xl font-black text-slate-900">{value}</h4>
    </div>
  );
}
...
function FeatureCard({ icon: Icon, title, desc }: { icon: any, title: string, desc: string }) {
  return (
    <div className="group p-6 md:p-8 rounded-xl md:rounded-2xl border bg-white hover:border-primary/50 transition-all duration-300">
      <div className="w-10 h-10 md:w-12 md:h-12 bg-primary/10 text-primary rounded-lg flex items-center justify-center mb-4 md:mb-6 group-hover:bg-primary group-hover:text-white transition-colors">
